...
import file_copy
from convert_mp4 import ConvertFromMP4
from bannerFormatChecker import CheckBannerFormat
from system_resize import ResizeFile
...
class App(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.dateforCatalog1_ = ctk.CTkLabel(master=self, textvariable=self.stringvar, width=322, height=25,
                                             text_color='#ffffff',
                                             fg_color='#1d1e1e', wraplength=322, justify='left', corner_radius=5,
                                             padx=5, pady=5, font=('Open Sans', 10))
        self.switch_var_0 = tk.StringVar(value="on")
        self.switch_var_1 = tk.StringVar(value="off")
        self.switch_var_2 = tk.StringVar(value="off")
        self.switch_var_3_PS = tk.StringVar(value="off")
        self.switch_0 = ctk.CTkSwitch(master=self, text="b _mb", variable=self.switch_var_0, onvalue="on",
                                      offvalue="off",
                                      progress_color='green', fg_color='red', switch_height=13, font=('Open Sans', 14),
                                      text_color='#ffffff')
        self.switch_1 = ctk.CTkSwitch(master=self, text=".webp", variable=self.switch_var_1, onvalue="on",
                                      offvalue="off",
                                      progress_color='green', fg_color='red', switch_height=13, font=('Open Sans', 14),
                                      text_color='#ffffff')
        ...
        self.switch_3_PS = ctk.CTkSwitch(master=self, text="PS", variable=self.switch_var_3_PS, onvalue="on",
                                         offvalue="off",
                                         progress_color='green', fg_color='red', switch_height=13,
                                         font=('Open Sans', 14),
                                         text_color='#ffffff')
        self.var = tk.IntVar()
        ...
        self.R1 = ctk.CTkRadioButton(master=self, text="610x242", variable=self.var, value=610, hover_color='#d11507',
                                     ...
                                     border_width_checked=2, border_width_unchecked=2, fg_color='#d11507',
                                     text_color='#ffffff')
        ...
        self.button_2 = ctk.CTkButton(master=self, text='Resize', width=80, height=26, fg_color="green",
                                      hover_color='#34661e',
                                      font=('Open Sans', 12), command=self.resize_banner, state="disabled",
                                      text_color='#ffffff')
        self.button_3 = ctk.CTkButton(master=self, text='Resend', width=80, height=26, fg_color="#0033FF",
                                      hover_color='#0000FF',
                                      font=('Open Sans', 12), command=self.send_file, state="disabled",
                                      text_color='#ffffff')
        self.button_4 = ctk.CTkButton(self, text='Connection', width=80, height=26, fg_color="#0033FF",
                                      hover_color='#0000FF',
                                      font=('Open Sans', 12), command=self.connection_run, text_color='#ffffff')

        self.button_5 = ctk.CTkButton(master=self, text='Exit', width=80, height=26, fg_color="#e33118",
                                      hover_color='#d11507',
                                      font=('Open Sans', 12), command=self.quite_app, text_color='#ffffff')

        self.customeDimention_W = ctk.CTkEntry(master=self, corner_radius=5, border_color='green', border_width=1,
                                               width=60, height=10, fg_color='#1d1e1e', text_color='#ffffff',
                                               placeholder_text="Width")
        self.customeDimention_H = ctk.CTkEntry(master=self, corner_radius=5, border_color='green', border_width=1,
                                               width=60, height=10, fg_color='#1d1e1e', text_color='#ffffff',
    ...
    def create_layout(self):
        ...
        self.switch_3_PS.place(x=220, y=160, anchor='center')
        ...
        # W trakcie będzie dawało możliwośc wyboru czy chcemy domyślen skalowanie czy sami okreśłimy parametry sklaowania
        # self.customeDimention_W.place(x=470, y=162, anchor='center')
        # self.customeDimention_H.place(x=400, y=162, anchor='center')

    def custome_dimention(self):
        width = self.customeDimention_W.get().strip()
        height = self.customeDimention_H.get().strip()
        return [width, height]
    ...
    def switch_event_3_PS(self):
        self.switch_data_3_PS = self.switch_var_3_PS.get()
        return self.switch_data_3_PS
    ...
    def resize_banner(self):
        if self.ban_dir is not None and self.baner_size_data is not None:
            ...
            is_PS_check = self.switch_event_3_PS()
            is_b_bm_on = self.switch_event_0()
            is_sd_on = self.switch_event_2()
            self.prepare_and_send(int(dimensions), is_b_bm_on, is_sd_on, is_PS_check)

    def prepare_and_send(self, dimensions, is_b_bm_on, is_sd_on, photoshop_or_not):
        ...
        for root, dir, files in os.walk(self.ban_dir):
            ...
            self.scaled_file(root, file_name, dimensions, is_b_bm_on, is_sd_on,  photoshop_or_not)
    ...
    def clean_name_space(self, root, files):
        plik_jpg_istnieje = any(
            plik.endswith(rozszerzenie) for plik in files for rozszerzenie in ['.jpg', '.png', '.mp4', '.gif'])
    ...
    def scaled_file(self, root, file_name, dimensions, is_b_bm_on, is_sd_on, photoshop_or_not):
        banner_format = CheckBannerFormat(root)
        self.banner_list: list = list(os.listdir(f"{root}"))
        match photoshop_or_not:
            case "on":
                photoShopAPi = aps.PhotoshopAPI(root, file_name, dimensions, is_b_bm_on, is_sd_on)
                photoShopAPi.psBannerResizer()
            case "off":
                for banner_name in self.banner_list:
                    if banner_name.endswith('.mp4'):
                        print('znalazło mp4')
                        ConvertFromMP4(root)
                        ResizeFile(catDir=root, data=file_name, width_height=dimensions, b_mb=is_b_bm_on,
                                   is_sd_on=is_sd_on)
                    else:
                        print('nie znalazło pm4')
                        print(self.banner_list)
                        ResizeFile(catDir=root, data=file_name, width_height=dimensions, b_mb=is_b_bm_on, is_sd_on=is_sd_on)
    ...
    def copy_dach_fr(self, bandir, data, ext):
        ...
        f_cop.file_copy_bef(data, ext)
        f_cop.file_copy_ben(data, ext)
